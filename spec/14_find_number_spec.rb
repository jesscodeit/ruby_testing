...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:five_nine) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        guess = five_nine.make_guess
        expect(guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:eight_nine) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        guess = eight_nine.make_guess
        expect(guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:zero_three) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        guess = zero_three.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:three_three) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        guess = three_three.make_guess
  ...
  describe '#game_over?' do
    let(:the_answer) { double('the_answer', value: 8) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:equal_numbers) { described_class.new(0, 9, the_answer, 8) }
      ...
      it 'is game over' do
        expect(equal_numbers).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:not_equal) { described_class.new(0, 9, the_answer, 7) }
      
      it 'is not game over' do
        expect(not_equal).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:last_guess_game) { described_class.new(5, 8, number_range, 7) }

      before do
        last_guess_game.update_range
      end

      it 'updates min to the same value as max' do
        new_min = last_guess_game.min
        expect(new_min).to eq(8)
      end

      it 'does not update max' do
        new_max = last_guess_game.max
        expect(new_max).to eq(8)
      end
    end
  end
