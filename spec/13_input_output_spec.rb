...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      subject(:game_not_over) { described_class.new(4, '2') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        word = 'one'
        valid_input = '5'
        allow(game_loop).to receive(:player_input).and_return(letter, word, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      subject(:game_three) { described_class.new(7, '7', 3) }
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game_four) { described_class.new(8, '8', 8) }
      it 'outputs correct phrase' do
        yowtch_phrase = "That was hard. It took you 8 guesses!\n"
        expect { game_four.final_message }.to output(yowtch_phrase).to_stdout
      end
    end
  end
end
