...
describe Cat do
  ...
  subject(:freyja) { described_class.new('Freyja', 'Siamese', 'Seal Point') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when Cat is child of class of Pet' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(freyja).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(freyja).to be_hiding
  end
end
