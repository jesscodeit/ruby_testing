...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 20 and the random number is 13' do 
      subject(:leka) { described_class.new(13) }

      it 'returns 7' do
        previous_step = 20
        result = leka.subtract_random_number(previous_step)
  ...
  describe '#play' do
    ...
    context 'when the random number is 10' do
      ...
      subject(:ten) { described_class.new(10) }
    ...
    context 'when the random number is 42' do
      # remove the 'x' before running this test
      subject(:forty_two) { described_class.new(42) }

      it 'will return 7' do
        result = forty_two.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 375' do
      # remove the 'x' before running this test
      subject(:meep) { described_class.new(375) }
